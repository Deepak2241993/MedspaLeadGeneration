...
@section('content')
    ...
    @push('scripts')
        ...
        {{-- <script>
            // Function to make a call to selected phone numbers
            function makeCallToSelectedNumbers() {
                var selectedPhoneNumbers = getSelectedPhoneNumbers();
                if (selectedPhoneNumbers.length > 0) {
                    $.ajax({
                        ...
                        error: function(xhr, status, error) {
                            console.error(xhr.responseText);
                            alert('Failed to initiate calls. Please try again later.');
                        }
                    });
                } else {
                    alert('Please select at least one row to make a call.');
                }
            }

            // Function to get selected phone numbers from DataTable rows
            function getSelectedPhoneNumbers() {
                var selectedPhoneNumbers = [];
                $('.row-checkbox:checked').each(function() {
                    var phoneNumber = $(this).closest('tr').find('.phone-column').text().trim();
